...
3. **`doordash_dashboard.main()`** orchestrates:
   - `_render_header_banner()` → `_render_sidebar_upload()` → `_handle_run_analysis_click()`
   - `_render_airtable_sidebar_filters()` → `_compute_filtered_mapping_df()` → `_rebuild_mapping_matrix_if_filters()`
